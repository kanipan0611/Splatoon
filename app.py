...
with st.sidebar:
    ...
    api_key_input = st.text_input(
        ...
        help="AI動画分析・AIコーチに必要。未設定でも試合記録と知識ベースは使えます。",
    )
    if api_key_input:
        os.environ["ANTHROPIC_API_KEY"] = api_key_input

    if ai_coach.has_api_key():
        st.success("APIキー設定済み — AI機能が使えます")
    else:
        st.info("APIキー未設定 — AI機能はオフライン版(定型アドバイス)で動作します")
...
# --- 🎬 動画分析 --------------------------------------------------------------
with tab_video:
    st.subheader("プレイ動画のAI分析")
    st.markdown(
        "プレイ動画(MP4/MOV)をアップロードすると、フレームを抽出してAIコーチが"
        "**良かった点・改善点・練習メニュー**をレポートします。"
    )
        ...
        if st.button("分析する", type="primary"):
            ...
            try:
                with st.spinner("フレームを抽出中..."):
                    ...
                    frames = select_frames_for_ai(analysis)
                ...
                if key_frames:
                    st.markdown("**注目シーン候補**(画面変化が大きい瞬間 = キル/デス/スペシャル等の可能性)")
                ...
                if ai_coach.has_api_key():
                    with st.spinner("AIコーチが分析中...(1〜2分かかることがあります)"):
                        report = ai_coach.analyze_video_frames(frames, context)
                    st.markdown("---")
                    st.markdown(report)
                else:
                    st.warning(
                        "APIキー未設定のためAI分析はスキップしました。"
                        "代わりに選択したルール・ブキ種に応じた定型アドバイスを表示します。"
                    )
                    st.markdown(
...
with tab_chat:
    ...
    if prompt := st.chat_input("例: ガチエリアで打開がうまくいきません。ブキはスシです。"):
        ...
        with st.chat_message("assistant"):
            if ai_coach.has_api_key():
                with st.spinner("考え中..."):
                    reply = ai_coach.chat(st.session_state.chat_history)
            else:
                # オフライン: キーワードからルール/ブキ種を推定して定型アドバイス
                mode = next((m for m in knowledge.MODES if m.replace("バトル", "") in prompt), None)
                wclass = next((w for w in knowledge.WEAPON_CLASSES if w in prompt), None)
                reply = (
                    "(オフライン版の回答です。APIキーを設定するとAIが個別にアドバイスします)\n\n"
...
with tab_stats:
    ...
    if df.empty:
        ...
        if ai_coach.has_api_key() and total >= 5:
            if st.button("🤖 AIに戦績の傾向を分析してもらう"):
                with st.spinner("分析中..."):
                    st.markdown(ai_coach.analyze_match_stats(log.summary_text()))
        elif total >= 5:
            st.caption("APIキーを設定すると、戦績データのAI傾向分析が使えます。")
...
# --- 📚 知識ベース ------------------------------------------------------------
with tab_knowledge:
    st.subheader("立ち回り知識ベース(APIキー不要)")
